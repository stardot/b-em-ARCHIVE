...
int sound_internal = 0, sound_beebsid = 0, sound_dac = 0, sound_ddnoise = 0,
    sound_tape = 0;
int sound_filter = 0;
...
{
	float ACoef[NCoef + 1] = {
	    0.30631912757971225000, 0.00000000000000000000,
	    -0.61263825515942449000, 0.00000000000000000000,
	    0.30631912757971225000};

	float BCoef[NCoef + 1] = {
	    1.00000000000000000000, -1.86772356053227330000,
	    1.08459167506874430000, -0.37711292573951394000,
...
void sound_poll()
{
	...
	if (sound_pos == (BUFLEN >> 1)) {
		if (BUFLEN & 1) {
			if (sound_beebsid)
				sid_fillbuf(sound_buffer + (sound_pos << 1), 1);
			if (sound_internal)
				sn_fillbuf(sound_buffer + (sound_pos << 1), 1);
			if (sound_dac)
