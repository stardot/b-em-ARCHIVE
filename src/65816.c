...
static struct {
	int c, z, i, d, b, v, n, m, ex,
	    e; /*X renamed to EX due to #define conflict*/
} w65816p;
...
static uint16_t readmemw65816(uint32_t a)
{
	...
	//        cycles-=2;
	//        rpclog("Reading %08X %i %08X
	//        %08X\n",a,tuberomin,w65816rom,w65816ram);
	//        if ((a&~0xFFF)==0xF000 && tuberomin) return
	//        w65816rom[a&0xFFF]|(w65816rom[(a+1)&0xFFF]<<8);
...
static uint32_t jindirectx() /*JSR (,x) uses PBR instead of DBR, and 2 byte
...
static void decZp8()
{
	...
	setzn8(temp);
	writemem(addr, temp);
	//        if (output && addr==4) printf("DEC 4 %02X %i
	//        %i\n",temp,p.z,p.n);
}
static void decZp16()
{
...
static void stp() /*No point emulating this properly as the external support
...
void w65816_exec()
{
	while (tubecycles > 0) {
		...
		pc++;
		if (toutput)
			rpclog("%i : %02X:%04X %04X %02X %i %04X  %04X %04X "
			       "%04X\n",
