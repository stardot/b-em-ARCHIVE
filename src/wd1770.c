...
void wd1770_write(uint16_t addr, uint8_t val)
{
	...
	case 0xFE28:
		...
		case 0xA: /*Write sector*/
			wd1770.status = 0x80 | 0x1;
			disc_writesector(curdrive, wd1770.sector, wd1770.track,
					 wd1770.curside, wd1770.density);
			byte = 0;
			nmi |= 2;
			wd1770.status |= 2;

			// Carlo Concari: wait for first data byte before
			// starting sector write
			wd1770.written = 0;
			break;
		case 0xC: /*Read address*/
			wd1770.status = 0x80 | 0x1;
			disc_readaddress(curdrive, wd1770.track, wd1770.curside,
					 wd1770.density);
			byte = 0;
			break;
