...
void main_init(int argc, char *argv[])
{
	...
	for (c = 1; c < argc; c++) {
		...
		if (!strcasecmp(argv[c], "--help")) {
			...
			printf("-mx             - start as model x (see "
			       "readme.txt for models)\n");
			printf("-tx             - start with tube x (see "
			       "readme.txt for tubes)\n");
			printf("-disc disc.ssd  - load disc.ssd into drives "
			       ":0/:2\n");
			printf("-disc1 disc.ssd - load disc.ssd into drives "
			       ":1/:3\n");
			...
			printf("-allegro        - use Allegro for video "
			       "rendering\n");
			exit(-1);
		} else
		    //#endif
			...
			discnext = 2;
			   ...
			   (argv[c][1] == 'm' || argv[c][1] == 'M')) {
			sscanf(&argv[c][2], "%i", &curmodel);
		} else if (argv[c][0] == '-' &&
			   (argv[c][1] == 't' || argv[c][1] == 'T')) {
			...
			if (vid_fskipmax > 9)
			   ...
			   (argv[c][1] == 's' || argv[c][1] == 'S')) {
			vid_scanlines = 1;
		} else if (!strcasecmp(argv[c], "-debug")) {
			debug = debugon = 1;
		} else if (argv[c][0] == '-' &&
			   (argv[c][1] == 'i' || argv[c][1] == 'I')) {
			vid_interlace = 1;
		} else if (tapenext)
