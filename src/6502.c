...
#define polltime(c)                                       \
	{                                                 \
		...
		if (!(sysvia.acr & 0x20))                 \
			sysvia.t2c -= (c);                \
		if (sysvia.t1c < -3 || sysvia.t2c < -3)   \
			sysvia_updatetimers();            \
		uservia.t1c -= (c);                       \
		if (!(uservia.acr & 0x20))                \
			uservia.t2c -= (c);               \
		if (uservia.t1c < -3 || uservia.t2c < -3) \
			uservia_updatetimers();           \
		video_poll(c);                            \
		otherstuffcount -= (c);                   \
		if (motoron) {                            \
			if (fdc_time) {                   \
...
#define SBC(temp)                                                            \
	if (!p.d) {                                                          \
		...
		p.v = ((a ^ (temp + (p.c ? 0 : 1))) & (a ^ (uint8_t)tempv) & \
		       0x80);                                                \
		p.c = tempv >= 0;                                            \
		a = tempw & 0xFF;                                            \
		setzn(a);                                                    \
...
void m6502_exec()
{
	...
	while (cycles > 0) {
		...
		case 0x6B: /*Undocumented - ARR*/
			...
			tempi = p.c;
			if (p
				.d) /*This instruction is just as broken on a
				       real 6502 as it is here*/
			{
				p.v = ((a >> 6) ^
				       (a >>
					7)); /*V set if bit 6 changes in ROR*/
				a >>= 1;
				if (tempi)
					a |= 0x80;
				setzn(tempi);
				...
				if ((a & 0xF) + (a & 1) > 5)
					a = (a & 0xF0) +
					    ((a & 0xF) + 6); /*Do broken fixup*/
				if ((a & 0xF0) + (a & 0x10) > 0x50) {
					a += 0x60;
					p.c = 1;
				}
			} else { /*V & C flag behaviours in 64doc.txt are
				    backwards*/
				p.v = ((a >> 6) ^
				       (a >>
					7)); /*V set if bit 6 changes in ROR*/
		...
		case 0x6C: /*JMP ()*/
			...
			if ((addr & 0xFF) == 0xFF)
				pc =
				    readmem(addr) | (readmem(addr - 0xFF) << 8);
			else
		...
		case 0x82: /*Undocumented - NOP imm*/ /*Should sometimes lock up
							 the machine*/
			readmem(pc);
			pc++;
			polltime(2);
		...
		case 0x8B: /*Undocumented - ANE*/
			temp = readmem(pc);
			pc++;
			a = (a | 0xEE) & x & temp; /*Internal parameter always
						      0xEE on BBC, always 0xFF
						      on Electron*/
			setzn(a);
			polltime(2);
			takeint = (interrupt && !p.i);
		...
		case 0xAB: /*Undocumented - LAX*/
			...
			pc++;
			a = x =
			    ((a | 0xEE) & temp); /*WAAAAY more complicated than
						    this, but it varies from
						    machine to machine anyway*/
			setzn(a);
			polltime(2);
			takeint = (interrupt && !p.i);
			break;
		...
		case 0xC2: /*Undocumented - NOP imm*/ /*Should sometimes lock up
							 the machine*/
			readmem(pc);
			pc++;
			polltime(2);
		...
		case 0xE1: /*SBC (,x)*/ /*This was missed out of every B-em
					   version since 0.6 as it was never
					   used!*/
			temp = readmem(pc) + x;
			pc++;
			addr = readmem(temp) | (readmem(temp + 1) << 8);
		...
		case 0xE2: /*Undocumented - NOP imm*/ /*Should sometimes lock up
							 the machine*/
			readmem(pc);
			pc++;
			polltime(2);
...
//                                pc+=2;
		...
		default: /*Halt!*/
			...
			polltime(100000); /*Actually lasts forever, but the
					     above code keeps executing HLT
					     forever*/
			break;
			//                                printf("Found bad
			//                                opcode
		...
		//                if (pc==0xCD7A) printf("CD7A from
		//                %04X\n",oldpc);
		//                if (pc==0xC565) printf("C565 from
		//                %04X\n",oldpc);
		...
		/*                if (output)
				{
		//                        #undef printf
					rpclog("A=%02X X=%02X Y=%02X S=%02X
		PC=%04X %c%c%c%c%c%c op=%02X %02X%02X\n",a,x,y,s,pc,(p.n)?'N':'
		',(p.v)?'V':' ',(p.d)?'D':' ',(p.i)?'I':' ',(p.z)?'Z':'
		',(p.c)?'C':' ',opcode,ram[0x29],uservia.ifr);
				}*/
...
void m65c02_exec()
{
	...
	while (cycles > 0) {
		//                if (pc==0x806F) rpclog("806F from %04X
		//                %04X\n",oldpc,oldoldpc);
		//                if (pc>0xDFF && pc<0x8000) rpclog("EXEC
		//                %04X\n",pc);
		pc3 = oldoldpc;
		oldoldpc = oldpc;
		oldpc = pc;
		...
		/*                if (output | 1)
				{
					rpclog("A=%02X X=%02X Y=%02X S=%02X
		   PC=%04X %c%c%c%c%c%c op=%02X %02X%02X %02X%02X %02X
		   %08X\n",a,x,y,s,pc,(p.n)?'N':' ',(p.v)?'V':' ',(p.d)?'D':'
		   ',(p.i)?'I':' ',(p.z)?'Z':' ',(p.c)?'C':'
		   ',opcode,ram[0x21],ram[0x20],ram[0x7F],ram[0x7E],ram[0x7D],memlook[pc>>8]);
				}*/
		/*                if (timetolive)
				{
					timetolive--;
