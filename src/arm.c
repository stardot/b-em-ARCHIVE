...
#define LOADREG(r, v)                                                   \
	if (r == 15) {                                                  \
		armregs[15] =                                           \
		    (armregs[15] & 0xFC000003) | ((v + 4) & 0x3FFFFFC); \
		refillpipeline();                                       \
	} else                                                          \
		armregs[r] = v;
#define GETREG(r) ((r == 15) ? armregs[15] + 4 : armregs[r])
#define LDRRESULT(a, v) \
	((a & 3) ? (v >> ((a & 3) << 3)) | (v << (((a & 3) ^ 3) << 3)) : v)
...
//#define readarml(a)
//((armread[(a>>20)&63])?armread[(a>>20)&63][(a&0xFFFFF)>>2]:readarmfl(a))
...
#define ldrresult(v, a) \
	((v >> ldrlookup[addr & 3]) | (v << (32 - ldrlookup[addr & 3])))
...
#define LDMallS()                                                          \
	...
	if (opcode & 0x8000) {                                             \
		...
		//                if (armregs[12]==0x1000000)
		//                rpclog("R12=1000000 %08X  %i\n",PC,armins);
		armirq = tube_irq;
		...
		//                if (output && !(*armregs[15]&0x8000000) &&
		//                PC<0x2000000) rpclog("%07X : %08X %08X %08X
		//                %08X %08X %08X %08X %08X\n%08i: %08X %08X %08X
		//                %08X %08X %08X %08X
