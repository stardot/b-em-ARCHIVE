...
void video_doblit()
{
	...
	if (vid_savescrshot) {
		...
		if (!vid_savescrshot) {
			set_color_depth(dcol);
			scrshotb = create_bitmap(lastx - firstx,
						 (lasty - firsty) << 1);
			scrshotb2 =
	...
	if (fskipcount >= ((motor && fasttape) ? 5 : vid_fskipmax)) {
		...
		if (videoresize && !fullscreen) {
			...
			if (vid_scanlines) {
				...
				pal_convert(
				    b, firstx,
				    (firsty << 1) + (interlline ? 1 : 0), lastx,
				    (lasty << 1) + (interlline ? 1 : 0), 2);
				blit(b32, vb, (firstx * 922) / 832, firsty << 1,
				     0, 0, ((lastx - firstx) * 922) / 832,
				     (lasty - firsty) << 1);
