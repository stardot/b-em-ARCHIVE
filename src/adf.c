...
void adf_writesector(int drive, int sector, int track, int side, int density)
{
	...
	if (adf_size[drive] != 256)
		adf_sector--;
	//        printf("ADFS Write sector %i %i %i
...
void adf_poll()
{
	...
	if (adf_inreadaddr && adf_f[adf_drive]) {
		...
		case 2:
			fdc_data(
			    adf_rsector + (adf_size[adf_drive] != 256) ? 1 : 0);
			break;
		case 3:
			fdc_data((adf_size[adf_drive] == 256)
				     ? 1
				     : ((adf_size[adf_drive] == 512) ? 2 : 3));
			break;
		case 4:
			fdc_data(0);
			break;
