...
',(tubep.d)?'D':' ',(tubep.i)?'I':' ',(tubep.z)?'Z':' ',(tubep.c)?'C':' ');
}*/
...
static void tubewritememl(uint16_t addr, uint8_t val)
{
	...
	if ((addr & ~7) == 0xFEF8) {
		//                rpclog("Write tube %04X %02X
		//                %04X\n",addr,val,pc);
		tube_parasite_write(addr, val);
		endtimeslice = 1;
		return;
	}
	//        if (addr==0xF4 || addr==0xF5) rpclog("TUBE PARASITE write %04X
	//        %02X\n",addr,val);
	tuberam[addr] = val;
}

#define readmem(a)                                      \
	((tubememstat[(a) >> 8] == 2) ? tubereadmeml(a) \
				      : tubemem[(a) >> 8][(a)&0xFF])
#define writemem(a, b) tubewritememl(a, b)
// if (tubememstat[(a)>>8]==0) tubemem[(a)>>8][(a)&0xFF]=b; else if
// (tubememstat[(a)>>8]==2) tubewritememl(a,b)
#define getw()                                  \
...
#define SBC(temp)                                                          \
	if (!tubep.d) {                                                    \
		...
		tubep.v = ((a ^ (temp + (tubep.c ? 0 : 1))) &              \
			   (a ^ (uint8_t)tempv) & 0x80);                   \
		tubep.c = tempv >= 0;                                      \
		a = tempw & 0xFF;                                          \
		setzn(a);                                                  \
	} else {                                                           \
...
void tube_6502_exec()
{
	...
	while (tubecycles > 0) {
		...
		/*                        if (tubeoutput==2) rpclog("%04X : %02X
		   %02X %02X\n",pc,a,x,y);
					if (tubetimetolive)
					{
						tubetimetolive--;
