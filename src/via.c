...
void via_write(VIA *v, uint16_t addr, uint8_t val)
{
	...
	case ORB:
		v->ifr &= ~INT_CB1;
		if ((v->pcr & 0xA0) !=
		    0x20) /*Not independent interrupt for CB2*/
			v->ifr &= ~INT_CB2;
		via_updateIFR(v);
	...
	case T2CL:
		...
		break;
	case T2CH:
		/*Fix for Kevin Edwards protection - if interrupt triggers in
		 * cycle before write then let it run*/
...
uint8_t via_read(VIA *v, uint16_t addr)
{
	...
	case ORAnh:
		...
		if (v->acr & 1)
			...
			temp |= (v->read_portA() &
				 ~v->ddra); /*Read current port values*/
		return temp;
	...
	case ORB:
		...
		if (v->acr & 2)
			...
			temp |= (v->read_portB() &
				 ~v->ddrb); /*Read current port values*/
		return temp;
