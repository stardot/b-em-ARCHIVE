...
static void s0a(FDI *fdi)
{
	...
	uae_u8 b;
	fdi->track_src += 2;
	debuglog("s0a:bits=%d,data=%s", bits,
		 datalog(fdi->track_src, (bits + 7) / 8));
	while (bits >= 8) {
...
static void s0b(FDI *fdi)
{
	...
	uae_u8 b;
	fdi->track_src += 2;
	debuglog("s0b:bits=%d,data=%s", bits,
		 datalog(fdi->track_src, (bits + 7) / 8));
	while (bits >= 8) {
		byte_add(fdi, *fdi->track_src++);
		bits -= 8;
...
static void s0c(FDI *fdi)
{
	...
	fdi->track_src += 2;
	bit_drop_next(fdi);
	debuglog("s0c:bits=%d,data=%s", bits,
		 datalog(fdi->track_src, (bits + 7) / 8));
	while (bits >= 8) {
...
static void s0d(FDI *fdi)
{
	...
	fdi->track_src += 2;
	bit_drop_next(fdi);
	debuglog("s0d:bits=%d,data=%s", bits,
		 datalog(fdi->track_src, (bits + 7) / 8));
	while (bits >= 8) {
		byte_mfm_add(fdi, *fdi->track_src++);
		bits -= 8;
...
{
	bit_drop_next(fdi);
	debuglog("s20:header=%s,data=%s", datalog(fdi->track_src, 4),
...
{
	bit_drop_next(fdi);
	debuglog("s22:sector=%d,untilgap=%d", fdi->track_src[0],
		 fdi->track_src[1]);
...
static void s24(FDI *fdi)
{
	...
	debuglog("s24:shift=%d,data=%s", shift,
...
{
	int shift = *fdi->track_src++;
	debuglog("s25:shift=%d,crc=%s,data=%s", shift,
		 datalog(fdi->track_src, 4),
		 datalog(fdi->track_src + 4, 128 << shift));
...
static void ibm_sector_header(FDI *fdi, uae_u8 *data, uae_u8 *crc, int secnum,
			      int pre)
{
	uae_u8 secbuf[5];
	uae_u8 crcbuf[2];
...
{
	debuglog("s17:header=%s,crc=%s", datalog(fdi->track_src, 4),
...
{
	debuglog("s18:sector=%d,header=%s", *fdi->track_src,
...
{
	int shift = *fdi->track_src++;
	debuglog("s1a:shift=%d,data=%s", shift,
		 datalog(fdi->track_src, 128 << shift));
...
{
	int shift = *fdi->track_src++;
	debuglog("s1b:shift=%d,crc=%s,data=%s", shift,
...
static decode_described_track_func decode_sectors_described_track[] = {
    s00, s01, s02, s03, s04, dxx, dxx, dxx, s08,
    s09, s0a, s0b, s0c, s0d, dxx, dxx, /* 00-0F */
    s10, s11, s12, s13, s14, s15, s16, s17, s18,
    s19, s1a, s1b, s1c, s1d, dxx, dxx, /* 10-1F */
    s20, s21, s22, s23, s24, s25, s26, s27, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 20-2F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 30-3F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 40-4F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 50-5F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 60-6F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 70-7F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 80-8F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* 90-9F */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* A0-AF */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* B0-BF */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* C0-CF */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* D0-DF */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, /* E0-EF */
    dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx, dxx,
    dxx, dxx, dxx, dxx, dxx, dxx, sff /* F0-FF */
};
...
static int handle_sectors_described_track(FDI *fdi)
{
	...
	do {
		...
		outlog("%06.6X %06.6X %02.2X:",
		       fdi->track_src - start_src + 0x200, fdi->out / 8,
		       fdi->track_type);
		oldout = fdi->out;
...
static void init_array(uint32_t standard_MFM_2_bit_cell_size, int nb_of_bits)
{
	...
	for (i = 0; i < FDI_MAX_ARRAY; i++) {
		psarray[i].size =
		    standard_MFM_2_bit_cell_size; // That is (total track length
						  // / 50000) for Amiga double
						  // density
			...
			int *indexoffsetp, int pulses, int mfm)
{
	...
	/* detects a long-enough stable pulse coming just after another stable
	 * pulse */
	i = 1;
	while ((i < pulses) &&
	       ((idx[i] < maxidx) || (idx[i - 1] < maxidx) ||
		(minp[i] < (standard_MFM_2_bit_cell_size -
	...
	outstep = -1;
	while (outstep < 2) {
		/* calculates the current average bitrate from previous decoded
		 * data */
		uae_u32 avg_size =
		    (total << (2 + mfm)) /
		    totaldiv; /* this is the new average size for one MFM bit */
			      /* uae_u32 avg_size =
			       * (uae_u32)((((float)total)*((float)(mfm+1))*4.0) /
			       * ((float)totaldiv)); */
		...
		/* this	is to prevent the average value from going too far
		* from the theoretical value, otherwise it could progressively
		* go to (2 *
		* real value), or (real value / 2), etc. */

		/* gets the next long-enough pulse (this may require more than
		 * one pulse) */
		pulse = 0;
		while (pulse < ((avg_size / 4) - (avg_size / 16))) {
			...
			if (idx[i] >= maxidx) { /* stable pulse */
				...
				if ((maxp[nexti] - avgp[nexti]) <
				    (avg_pulse - min_pulse))
					min_pulse = avg_pulse -
						    (maxp[nexti] - avgp[nexti]);
				if ((avgp[nexti] - minp[nexti]) <
				    (max_pulse - avg_pulse))
					max_pulse = avg_pulse +
						    (avgp[nexti] - minp[nexti]);
				...
				if ((avg_pulse < min_pulse) ||
				    (avg_pulse > max_pulse)) {
					outlog("FDI: avg_pulse outside bounds! "
					       "avg=%u min=%u max=%u\n",
					       avg_pulse, min_pulse, max_pulse);
					outlog("FDI: avgp=%u (%u) minp=%u (%u) "
					       "maxp=%u (%u) jitter=%d i=%d "
					       "ni=%d\n",
					       avgp[i], avgp[nexti], minp[i],
					       minp[nexti], maxp[i],
					       maxp[nexti], jitter, i, nexti);
				}
				if (avg_pulse < ref_pulse)
					outlog("FDI: avg_pulse < ref_pulse! "
					       "(%u < %u)\n",
					       avg_pulse, ref_pulse);
		...
		/* gets the size in bits from the pulse width, considering the
			    ...
			    uae_u16 *out)
{
	...
	for (i = 0; i < bitoffset / 8; i++) {
		double v = (pt2[0] + pt2[1] + pt2[2] + pt2[3] + pt2[4] +
			    pt2[5] + pt2[6] + pt2[7]) /
			   8.0;
...
static int decode_lowlevel_track(FDI *fdi, int track, struct fdi_cache *cache)
{
	...
	if (get_u24(p1 + 3) && get_u24(p1 + 6)) {
		minp = (uae_u32 *)fdi_decompress(pulses, p1 + 3, p1 + len,
						 &min_free);
		len += get_u24(p1 + 3) & 0x3fffff;
		maxp = (uae_u32 *)fdi_decompress(pulses, p1 + 6, p1 + len,
						 &max_free);
	...
	p1 = idxp;
	for (i = 0; (i < pulses) && (p1[idx_off2] != 0);
	     i++) /* falling edge, replace with idx_off1 for rising edge */
		p1 += idx_off3;
	if (i < pulses) {
		j = i;
		do {
		...
		} while ((i != j) && (p1[idx_off2] == 0)); /* falling edge,
							      replace with
							      idx_off1 for
							      rising edge */
		if (i != j) /* index pulse detected */
		{
			while ((i != j) &&
			       (p1[idx_off1] >
				p1[idx_off2])) { /* falling edge, replace with
						    "<" for rising edge */
				i++;
				p1 += idx_off3;
				if (i >= pulses) {
...
FDI *fdi2raw_header(FILE *f)
{
	...
	if ((fdi->header[140] != 1 && fdi->header[140] != 2) ||
	    (fdi->header[141] != 0 &&
	     !(fdi->header[140] == 2 && fdi->header[141] == 1))) {
		fdi_free(fdi);
		return NULL;
	}
...
int fdi2raw_loadrevolution_2(FDI *fdi, uae_u16 *mfmbuf, uae_u16 *tracktiming,
			     int track, int *tracklength, int *indexoffsetp,
			     int *multirev, int mfm)
{
	...
	fdi2_decode(fdi, cache->totalavg, cache->avgp, cache->minp, cache->maxp,
		    cache->idxp, cache->maxidx, &idx, cache->pulses, mfm);
	// fdi2_gcr_decode (fdi, totalavg, avgp, minp, maxp, idxp, idx_off1,
	// idx_off2, idx_off3, maxidx, pulses);
	outlog("track %d: nbits=%d avg len=%.2f weakbits=%d idx=%d\n", track,
	       bitoffset, (double)cache->totalavg / bitoffset, cache->weakbits,
	       cache->indexoffset);
	len = fdi->out;
		      ...
		      int *multirev, int mfm)
{
	...
	if (outlen > 0) {
		if (cache->lowlevel)
			return fdi2raw_loadrevolution_2(
			    fdi, mfmbuf, tracktiming, track, tracklength,
			    indexoffsetp, multirev, mfm);
		*tracklength = fdi->out;
