...
void fdi_seek(int drive, int track)
{
	...
	c = fdi2raw_loadtrack(
	    fdi_h[drive], (uint16_t *)fdi_trackinfo[drive][0][0],
	    (uint16_t *)fdi_timing, track << fdi_sides[drive],
	    &fdi_tracklen[drive][0][0], &fdi_trackindex[drive][0][0], NULL, 0);
	if (!c)
		memset(fdi_trackinfo[drive][0][0], 0,
		       fdi_tracklen[drive][0][0]);
	c = fdi2raw_loadtrack(
	    fdi_h[drive], (uint16_t *)fdi_trackinfo[drive][0][1],
	    (uint16_t *)fdi_timing, track << fdi_sides[drive],
	    &fdi_tracklen[drive][0][1], &fdi_trackindex[drive][0][1], NULL, 1);
	if (!c)
		memset(fdi_trackinfo[drive][0][1], 0,
		       fdi_tracklen[drive][0][1]);
	if (fdi_sides[drive]) {
		c = fdi2raw_loadtrack(
		    fdi_h[drive], (uint16_t *)fdi_trackinfo[drive][1][0],
		...
		c = fdi2raw_loadtrack(
		    fdi_h[drive], (uint16_t *)fdi_trackinfo[drive][1][1],
		    (uint16_t *)fdi_timing, (track << fdi_sides[drive]) + 1,
	...
	//        printf("SD Track %i Len %i Index %i
	//        %i\n",track,ftracklen[drive][0][0],ftrackindex[drive][0][0],c);
	//        printf("DD Track %i Len %i Index %i
...
void fdi_poll()
{
	...
	tempi = fdi_trackinfo[fdi_drive][fdi_side][fdi_density]
			     [((fdi_pos >> 3) & 0xFFFF) ^ 1] &
		(1 << (7 - (fdi_pos & 7)));
	fdi_pos++;
	fdi_buffer <<= 1;
	...
	if (pollbitsleft) {
		...
		if (!pollbitsleft) {
			...
			if (pollbytesleft)
				pollbitsleft =
				    16; /*Set up another word if we need it*/
			if (readidpoll) {
				...
				if (!pollbytesleft) {
					if ((fdi_sectordat[0] == fdi_track &&
					     fdi_sectordat[2] == fdi_sector) ||
					    fdi_inreadaddr) {
						crc = (fdi_density) ? 0xcdb4
								    : 0xffff;
						calccrc(0xFE);
						for (c = 0; c < 4; c++)
							calccrc(
