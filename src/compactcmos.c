...
void compactcmos_i2cchange(int nuclock, int nudata)
{
	...
	//        log("I2C update clock %i %i data %i %i state
	//        %i\n",i2c_clock,nuclock,i2c_data,nudata,i2c_state);
	switch (i2c_state) {
	case I2C_IDLE:
	...
	case I2C_RECIEVE:
		...
		} else if (i2c_clock && nuclock && nudata &&
			   !lastdata) /*Stop bit*/
		{
	...
	case I2C_TRANSACKNOWLEDGE:
		if (!i2c_clock && nuclock) {
			if (nudata) /*It's not acknowledged - must be end of
				       transfer*/
			{
				//                                rpclog("End of
				//                                transfer\n");
