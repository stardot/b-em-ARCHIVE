...
void gui_update()
{
	...
	for (x = 0; x < 5; x++)
		tubemenu[x].flags =
		    (selecttube == (int)tubemenu[x].dp) ? D_SELECTED : 0;
#else
	for (x = 0; x < 4; x++)
		tubemenu[x].flags =
		    (selecttube == (int)tubemenu[x].dp) ? D_SELECTED : 0;
	...
	for (x = 0; x < 5; x++)
		tubespdmenu[x].flags =
		    (tube_6502_speed == (int)tubespdmenu[x].dp) ? D_SELECTED
								: 0;
	...
	for (x = 0; x < 3; x++)
		bordersmenu[x].flags =
		    (vid_fullborders == (int)bordersmenu[x].dp) ? D_SELECTED
								: 0;
	...
	for (x = 0; x < 5; x++)
		waveformmenu[x].flags =
		    (curwave == (int)waveformmenu[x].dp) ? D_SELECTED : 0;
	for (x = 0; x < 14; x++)
		sidtypemenu[x].flags =
		    (cursid == (int)sidtypemenu[x].dp) ? D_SELECTED : 0;
	...
	for (x = 0; x < 3; x++)
		ddvolmenu[x].flags =
		    (ddnoise_vol == (int)ddvolmenu[x].dp) ? D_SELECTED : 0;
	keymenu[1].flags = (keyas) ? D_SELECTED : 0;
	mousemenu[0].flags = (mouse_amx) ? D_SELECTED : 0;
	for (x = 0; x < 10; x++)
		speedmenu[x].flags =
		    (emuspeed == (int)speedmenu[x].dp) ? D_SELECTED : 0;
...
int gui_loadss()
{
	...
	ret = file_select_ex("Please choose a save state", tempname, "SNP", 260,
			     xsize, ysize);
	if (ret) {
...
MENU filemenu[6] = {{"&Return", gui_return, NULL, 0, NULL},
...
int gui_load0()
{
	...
	int xsize = windx - 32, ysize = windy - 16;
	memcpy(tempname, discfns[0], 260);
	ret = file_select_ex("Please choose a disc image", tempname,
			     "SSD;DSD;IMG;ADF;ADL;FDI", 260, xsize, ysize);
	if (ret) {
		disc_close(0);
...
MENU discmenu[8] = {{"Load disc :&0/2...", gui_load0, NULL, 0, NULL},
...
MENU tapespdmenu[3] = {{"Normal", gui_normal, NULL, 0, NULL},
...
int gui_loadt()
{
	...
	ret = file_select_ex("Please choose a tape image", tempname, "UEF;CSW",
			     260, xsize, ysize);
	if (ret) {
		tape_close();
...
MENU tapemenu[] = {{"Load tape...", gui_loadt, NULL, 0, NULL},
...
MENU tubespdmenu[6] = {{"4mhz", gui_tubespd, NULL, 0, (void *)1},
...
MENU displaymenu[4] = {{"Line doubling", gui_linedbl, NULL, 0, NULL},
...
MENU bordersmenu[4] = {{"None", gui_borders, NULL, 0, (void *)0},
...
MENU videomenu[4] = {{"Display type", NULL, displaymenu, 0, NULL},
...
MENU methodmenu[3] = {{"Interpolating", gui_method, NULL, 0, (void *)0},
		      {"Resampling", gui_method, NULL, 0, (void *)1},
		      {NULL, NULL, NULL, 0, NULL}};

MENU residmenu[3] = {{"Model", NULL, sidtypemenu, 0, NULL},
...
MENU waveformmenu[6] = {{"Square", gui_waveform, NULL, 0, (void *)0},
...
MENU ddtypemenu[3] = {{"5.25", gui_ddtype, NULL, 0, (void *)0},
...
MENU ddvolmenu[4] = {{"33%", gui_ddvol, NULL, 0, (void *)1},
...
MENU soundmenu[11] = {{"Internal sound chip", gui_internalsnd, NULL, 0, NULL},
...
MENU keymenu[3] = {{"Redefine keyboard", gui_keydefine, NULL, 0, NULL},
...
MENU mousemenu[2] = {{"&AMX mouse", gui_mouseamx, NULL, 0, NULL},
		     {NULL, NULL, NULL, 0, NULL}};
...
MENU idemenu[2] = {{"&Enable IDE hard discs", gui_ide, NULL, 0, NULL},
		   {NULL, NULL, NULL, 0, NULL}};

MENU settingsmenu[8] = {{"&Model", NULL, modelmenu, 0, NULL},
...
int gui_scrshot()
{
	...
	int xsize = windx - 32, ysize = windy - 16;
	tempname[0] = 0;
	ret = file_select_ex("Please enter filename", tempname, "BMP", 260,
			     xsize, ysize);
	if (ret) {
...
MENU speedmenu[11] = {{"&10%", gui_speed, NULL, 0, (void *)0},
...
MENU miscmenu[3] = {{"&Speed", NULL, speedmenu, 0, NULL},
		    {"Save screenshot", gui_scrshot, NULL, 0, NULL},
		    {NULL, NULL, NULL, 0, NULL}};

MENU mainmenu[6] = {{"&File", NULL, filemenu, 0, NULL},
