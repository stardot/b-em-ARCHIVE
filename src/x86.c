...
#define readmemwl(s, a)                                              \
	((((s) + (a)) < 0xE0000) ? *(uint16_t *)(&x86ram[(s) + (a)]) \
				 : readmemwlx86(s, a))
...
reg = If mod=11,  (depending on data size, 16 bits/8 bits, 32 bits=extend 16 bit
registers)
...
static uint16_t readmemwlx86(uint32_t seg, uint32_t addr)
{
	...
	//        if (addr2<0xC0000) return *(uint16_t
	//        *)(&x86ram[addr2-0x40000]);
	if (addr2 > 0xF0000)
...
static void x86dumpregs()
{
	...
	/*        if (is386)
	   ...
	   mode\n",(msw&1)?((eflags&VM_FLAG)?"V86":"protected"):"real");
		...
		printf("CS : base=%06X limit=%04X
	   access=%02X\n",cs,_cs.limit,_cs.access);
		printf("DS : base=%06X limit=%04X
	   access=%02X\n",ds,_ds.limit,_ds.access);
		printf("ES : base=%06X limit=%04X
	   access=%02X\n",es,_es.limit,_es.access);
		if (is386)
		{
			printf("FS : base=%06X limit=%04X
	   access=%02X\n",fs,_fs.limit,_fs.access);
			printf("GS : base=%06X limit=%04X
	   access=%02X\n",gs,_gs.limit,_gs.access);
		}
		printf("SS : base=%06X limit=%04X
	   access=%02X\n",ss,_ss.limit,_ss.access);
		...
		if (is386)
		{
			printf("386 in %s mode   stack in %s
...
static void outb(uint16_t port, uint8_t val)
{
	...
	if ((port & ~0xF) == 0x80) {
		//                if (port!=0x8E || val!=0) printf("Tube write
		//                %02X %02X\n",port,val);
...
static void x86_dma()
{
	if (!(x86ena & 2))
		return;
	//        printf("Src %05X %04X:%04X  Dst %05X
...
startrep:
	...
	pc++;
	//        if (firstrepcycle && temp==0xA5) printf("REP MOVSW %06X:%04X
	//        %06X:%04X\n",ds,SI,es,DI);
	...
	case 0xAB: /*REP STOSW*/
		...
		if (c > 0) {
			...
			firstrepcycle = 1;
		//                printf("REP STOSW %04X:%04X %04X:%04X %04X
		//                %04X\n",CS,pc,ES,DI,AX,CX); }
		break;
	case 0xAC: /*REP LODSB*/
		if (c > 0) {
	...
	case 0xAE: /*REP SCASB*/
		...
		if ((c > 0) && (fv == ((flags & Z_FLAG) ? 1 : 0))) {
			temp2 = readmembl(es + DI);
			//                        if (x86output) printf("SCASB
			//                        %02X %c %02X %05X
			//                        ",temp2,temp2,AL,es+DI);
			setsub8(AL, temp2);
			//                        if (x86output && flags&Z_FLAG)
			//                        printf("Match %02X
			//                        %02X\n",AL,temp2);
			if (flags & D_FLAG)
				DI--;
...
/*                                if (ins==500000)
		...
		case 0x8E: /*MOV sreg,w*/
			   //                        if (x86output) printf("MOV
			   //                        %04X  ",pc);
			fetchea();
			//                        if (x86output) printf("%04X
			//                        %02X\n",pc,rmdat);
			switch (rmdat & 0x38) {
			case 0x00: /*ES*/
				tempw = geteaw();
		...
		case 0x9A: /*CALL FAR*/
			...
			pc = tempw;
			loadcs(tempw2);
			/*                        if ((msw&1) && !(_cs.access&4)
			   && ((CS&3)<(tempw3&3)))
						{
							printf("Call to
			   non-confirming inner segment!\n");
							x86dumpregs();
							exit(-1);
						}*/
		...
		case 0xC4: /*LES*/
			...
			tempw = readmemwl(easeg,
					  (eaaddr + 2) & 0xFFFF); // geteaw2();
			loadseg(tempw, &_es);
			tubecycles -= 18;
			break;
		case 0xC5: /*LDS*/
		...
		case 0xCD: /*INT*/
			...
			//                        %02X\n",AL);
			//                        if (temp==0x18 || temp==0x19)
			//                        { printf("INT %02X\n",temp);
			//                        x86output=1; }
			//                        printf("INT %02X %04X %04X
			//                        %04X
			//                        %04X\n",temp,AX,BX,CX,DX);
			/*                        if (temp==0x21) printf("INT 21
			   %04X %04X %04X %04X %04X:%04X %06X
			...
			//                        x86output=0;
			//                        if (temp==0x13 && AH==3)
			//                        printf("Write sector %04X:%04X
			//                        %05X\n",es>>4,BX,es+BX);
			/*                        if (temp==0x13 && (DL==0x80 ||
			   DL==0x81) && AH>0)
						{
							int13hdc();
						}
						else if (temp==0x13 && AH==2 &&
			   DL<2 && FASTDISC)
						{
							int13read();
						}
						else if (temp==0x13 && AH==3 &&
			   DL<2 && FASTDISC)
						{
							int13write();
						}
						else if (temp==0x13 && AH==4 &&
			   DL<2 && FASTDISC)
						{
							AH=0;
							flags&=~C_FLAG;
		...
		pc &= 0xFFFF;
		//                if (CS==0x1490 && pc==0x3BBA) rpclog("Here
		//                from %04X:%04X %08X
		//                %02X\n",oldcs,oldpc,old8,opcode);

		//                if (CS==0x6012 && pc==3) rpclog("XIOS direct
		//                call %02X %04X %04X %04X
		//                %04X\n",AL,CX,DX,BX,SI);
