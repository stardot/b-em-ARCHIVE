...
static float volslog[16] = {0.00000f, 0.59715f, 0.75180f, 0.94650f,
...
static int16_t snwaves[5][32] = {
    ...
    {-120, -112, -104, -96, -88, -80, -72, -64, -56, -48, -40, -32, -24, -16,
     -8, 0, 8, 16, 24, 32, 40, 48, 56, 64, 72, 80, 88, 96, 104, 112, 120, 127},
    {16, 32, 48, 64, 80, 96, 112, 128, 112, 96, 80, 64, 48, 32, 16, 0, -16, -32,
     -48, -64, -80, -96, -112, -128, -112, -96, -80, -64, -48, -32, -16, 0},
    {0, 24, 48, 70, 89, 105, 117, 124, 126, 124, 117, 105, 89, 70, 48, 24, 0,
     -24, -48, -70, -89, -105, -117, -124, -126, -124, -117, -105, -90, -70,
     -48, -25}};
...
void sn_fillbuf(int16_t *buffer, int len)
{
	...
	for (d = 0; d < len; d++) {
		...
		if (!(sn_noise & 4)) {
			if (curwave == 4)
				buffer[d] += (snwaves[4][sn_stat[0] & 31] *
					      volslog[sn_vol[0]]);
			else
				buffer[d] += (((sn_shift & 1) ^ 1) * 127 *
					      volslog[sn_vol[0]] * 2);
		} else
			buffer[d] += (((sn_shift & 1) ^ 1) * 127 *
				      volslog[sn_vol[0]] * 2);
