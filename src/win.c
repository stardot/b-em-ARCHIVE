...
void updatewindowtitle()
{
	if (curtube == 3 || mouse_amx) {
		if (!mousecapture)
			set_window_title("B-em v2.2 - click to capture mouse");
		else
			set_window_title(
			    "B-em v2.2 - CTRL-END to release mouse");
	} else
...

{
	...
	hwnd = CreateWindowEx(
	    ...
	    480 + (GetSystemMetrics(SM_CYFIXEDFRAME) * 2) +
		GetSystemMetrics(SM_CYMENUSIZE) +
				 ...
				 LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_FILE_LSTATE:
			EnterCriticalSection(&cs);
			if (!getfile(hwnd, "Save State (*.SNP)\0*.SNP\0All "
					   "files (*.*)\0*.*\0\0",
				     savestate_name)) {
				CheckMenuItem(hmenu, IDM_TUBE_6502 + selecttube,
					      MF_UNCHECKED);
		...
		case IDM_FILE_SSTATE:
			EnterCriticalSection(&cs);
			if (curtube != -1) {
				bem_error("Second processor save states not "
					  "supported yet.");
			} else {
				if (!getsfile(hwnd, "Save State "
						    "(*.SNP)\0*.SNP\0All files "
						    "(*.*)\0*.*\0\0",
					      savestate_name, "SNP"))
					savestate_save();
			}
		...
		case IDM_DISC_AUTOBOOT:
			if (!getfile(hwnd, "Disc image "
					   "(*.SSD;*.DSD;*.IMG;*.ADF;*.ADL;*."
					   "FDI)\0*.SSD;*.DSD;*.IMG;*.ADF;*."
					   "ADL;*.FDI\0All files (*.*)\0*.*\0",
				     discfns[0])) {
				doautoboot = 1;
			}
			break;
		...
		case IDM_DISC_EJECT_1:
			...
			break;
		case IDM_DISC_NEW_0:
			if (!getsfile(hwnd, "Disc image "
					    "(*.SSD;*.DSD;*.ADF;*.ADL)\0*.SSD;*"
					    ".DSD;*.ADF;*.ADL\0All files "
					    "(*.*)\0*.*\0",
				      discfns[0], "SSD")) {
				disc_close(0);
		...
		case IDM_DISC_WPROT_D:
			...
			CheckMenuItem(hmenu, IDM_DISC_WPROT_D,
				      (defaultwriteprot) ? MF_CHECKED
							 : MF_UNCHECKED);
			break;

		case IDM_TAPE_LOAD:
			if (!getfile(hwnd, "Tape image "
					   "(*.UEF;*.CSW)\0*.UEF;*.CSW\0All "
					   "files (*.*)\0*.*\0",
				     tape_fn)) {
				tape_close();
				tape_load(tape_fn);
		...
		case IDM_VIDEO_PALI:
			...
			CheckMenuItem(hmenu, LOWORD(wParam), MF_CHECKED);
			vid_scanlines = vid_interlace = vid_linedbl = vid_pal =
			    0;
		...
		case IDM_SCRSHOT:
			if (!getsfile(hwnd, "Bitmap file (*.BMP)\0*.BMP\0All "
					    "files (*.*)\0*.*\0",
				      vid_scrshotname, "BMP")) {
				vid_savescrshot = 1;
			}
	...
	case WM_LBUTTONUP:
		if (!mousecapture && (curtube == 3 || mouse_amx)) {
			...
			newclip.top += GetSystemMetrics(SM_CXFIXEDFRAME) +
				       GetSystemMetrics(SM_CYMENUSIZE) +
