...
/*6845 CRTC*/
uint8_t crtc[32];
static uint8_t crtc_mask[32] = {0xFF, 0xFF, 0xFF, 0xFF, 0x7F, 0x1F,
...
void crtc_write(uint16_t addr, uint8_t val)
{
	...
	if (!(addr & 1))
		...
		//                if (crtci!=12 && crtci!=13 && crtci!=14 &&
		//                crtci!=15) printf("Write CRTC R%i
		//                %02X\n",crtci,val);
	}
}
...
void mode7_makechars()
{
	...
	for (c = 0; c < 960; c++) {
		...
		for (d = 0; d < 16; d++) {
			mode7_graph[offs2 + d] =
			    (int)(((float)teletext_graphics[offs1 + x2] *
				   (1.0 - x)) +
				  ((float)teletext_graphics[offs1 + x2 + 1] *
				   x));
			mode7_sepgraph[offs2 + d] =
			    (int)(((float)
				       teletext_separated_graphics[offs1 + x2] *
				   (1.0 - x)) +
				  ((float)teletext_separated_graphics[offs1 +
								      x2 + 1] *
				   x));
			if (!d) {
				mode7_graph[offs2 + d] =
	...
	for (c = 0; c < 960; c++) {
		...
		for (d = 0; d < 16; d++) {
			mode7_chars[offs2 + d] =
			    (int)(((float)mode7_tempi2[offs1 + x2] *
				   (1.0 - x)) +
				  ((float)mode7_tempi2[offs1 + x2 + 1] * x));
			mode7_charsi[offs2 + d] =
			    (int)(((float)mode7_tempi[offs1 + x2] * (1.0 - x)) +
				  ((float)mode7_tempi[offs1 + x2 + 1] * x));
			x += (11.0 / 15.0);
			if (x >= 1.0) {
				x2++;
				x -= 1.0;
...
static inline void mode7_render(uint8_t dat)
{
	...
	if (dat < 0x20) {
		...
		if (mode7_dblx != mode7_dbl)
			dat = 32; /*Double height doesn't respect held
				     characters*/
...
void video_init()
{
	...
	col0 = collook[0] | (collook[0] << 8) | (collook[0] << 16) |
	       (collook[0] << 24);
	...
	for (c = 0; c < 16; c++) {
		for (d = 0; d < 64; d++) {
			mode7_lookup[d & 7][(d >> 3) & 7][c] = makecol(
			    (((d & 1) * c) * 255) / 15 +
				((((d & 8) >> 3) * (15 - c)) * 255) / 15,
			    ((((d & 2) >> 1) * c) * 255) / 15 +
				((((d & 16) >> 4) * (15 - c)) * 255) / 15,
...
void video_poll(int clocks)
{
	...
	while (clocks--) {
		...
		if (hc == crtc[2]) {
			...
			//                        if (interlace)
			//                        memset(b->line[(scry<<1)+interlline],0,scrx);
			//                        else
			//                        memset(b->line[scry],0,scrx);
			//                        scrx=0;
			scry++;
			if (scry >= 384) {
				scry = 0;
